...
jQuery(document).ready(function($){
	...
	jQuery(document).on('click', '.editing', function(e){
		e.preventDefault();
		editorcontent = jQuery(this).parent().find('textarea').val(); 
		if (jQuery(".wp-editor-wrap").hasClass("tmce-active")){
			tinyMCE.activeEditor.setContent(editorcontent);
		} else {
			jQuery('#customis').val(editorcontent);
		}
		jQuery('.editorappear').css({"left":50 + '%'});
		thisBlock = jQuery(this);
		
	});
	
	jQuery(document).on('click', '.finishedit', function(e){
		e.preventDefault();
		    if (jQuery(".wp-editor-wrap").hasClass("tmce-active")){
		        newcontent = tinyMCE.activeEditor.getContent();
		    }else{
		        newcontent = jQuery('#customis').val();
		    }
		console.log(thisBlock);
		thisBlock.parent().find('textarea').val(newcontent);
		jQuery('.editorappear').css({"left":-200 + '%'});
				
	});		
	jQuery(document).on('click', '.canceledit', function(e){
		e.preventDefault();
			if (jQuery(".wp-editor-wrap").hasClass("tmce-active")){
		        newcontent = tinyMCE.activeEditor.setContent('');
		    }else{
		        newcontent = jQuery('#customis').val('');
		    }
		jQuery('.editorappear').css({"left":-200 + '%'});
	...
	$( "ul.blocks" ).bind( "sortstop", function(event, ui) {
		...
		if (ui.item.hasClass('ui-draggable')) { 
		    ...
		    if(ui.item.hasClass('block-container')) {
		    	ui.item.find(">:first-child").remove(); 
		    	ui.item.find(">:first-child").addClass("visible-section");
		    }
